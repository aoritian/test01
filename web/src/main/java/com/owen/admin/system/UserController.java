...
public class UserController extends BaseController {
	...
	public JsonResult edit(User user,ModelMap map){
		try {
			...
			String[] role={"2"};
			userService.grant(user.getId(),role);
		} catch (Exception e) {
	...
	@ResponseBody
	@RequestMapping(value = "/grant2/{id}", method = RequestMethod.POST)
	public JsonResult test2(@PathVariable Integer id,String[] roleIds, ModelMap map) {
		try {
			userService.grant(id,roleIds);
		} catch (Exception e) {
			e.printStackTrace();
