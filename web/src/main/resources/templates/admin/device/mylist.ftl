                ...
                <div class="ibox-content">
                    <p>
                        	<@shiro.hasPermission name="system:user:add">
                                <button class="btn btn-success " type="button" onclick="add();"><i
...
<script>
    $(document).ready(function () {
        ...
        $("#table_list").bootstrapTable({
            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                    if (value == '0')
                        return '<span class="label label-warning">未封禁</span>';
                    return '<span class="label label-primary">已封禁</span>';
                }
            // }, {
            //     title: "出生日期",
            //     field: "birthday"
            ...
            //     }
                ...
                field: "empty",
                formatter: function (value, row, index) {
                    var operateHtml = '<@shiro.hasPermission name="system:user:edit"><button class="btn btn-primary btn-xs" type="button" onclick="edit(\''+ row.id +'\')"><i class="fa fa-edit"></i>&nbsp;编辑查看</button> &nbsp;</@shiro.hasPermission>';
    ...
    function edit(id) {
        layer.open({
            ...
            shade: false,
            area: ['893px', '600px'],
            content: '${ctx!}/admin/device/edit/' + id,
            end: function (index) {
    ...
    function del(id) {
        layer.confirm('确定删除吗?', {icon: 3, title: '提示'}, function (index) {
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "${ctx!}/admin/device/delete/" + id,
